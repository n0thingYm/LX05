...
namespace DAL
{
    public class Activity
    {
        ...
        public Model.Activity GetRecommend()
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "select * from activity where activtyVerify='审核通过' and recommend='是' and endTime>=now() order by recommendTime desc limit 1";
        ...
        public int Delete(int id)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "delete from activity where activity=@id";
